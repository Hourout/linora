...
class EarlyStopping():
    ...
    def update(self, batch, log):
        ...
        if self._params.monitor in log:
            ...
            if mode=='min':
                self.state = min(self._params.history[-patience:])+min_delta>self._params.history[-patience-1]
                if not self.state and self._params.baseline is not None:
                    self.state = min(self._params.history[-patience:])>baseline
            else:
                self.state = max(self._params.history[-patience:])-min_delta<self._params.history[-patience-1]
                if not self.state and self._params.baseline is not None:
                    self.state = max(self._params.history[-patience:])<baseline
